...
// Renderiza todos los muebles agrupados por OP.
        ...
        // En bares, el espesor estructural se toma desde los laterales.
        function obtenerGrosorLateralesDesdeItems(items) {
            const candidatos = [];

            items.forEach(row => {
                const pieza = normalizarPieza(val(row, "cod_pieza") || row.cod_pieza || row.codPieza || "");
                if (!esLateral(pieza)) return;

                const grosor = extraerGrosorMaterial(val(row, "jov") || row.jov || row.job || "");
                if (grosor) candidatos.push(grosor);
            });

            return candidatos.find(grosor => grosor > 0) || 0;
        }
